              ...
              output$map = renderLeaflet({
                ...
                leaflet(data = df_predictions) %>%
                  ...
                  addCircleMarkers(~Longitude, ~Latitude,
                                   # color = case_when(df_predictions$Delta / 1e3 < -5 ~ 'green', 
                                   #                   df_predictions$Delta / 1e3 > 5 ~ 'red',
                                   #                   TRUE ~ 'orange'),
              ...
              output$map = renderLeaflet({
                leaflet(data = house) %>%
                  ...
                  addCircleMarkers(
                    ...
                    color = case_when(house$Delta / 1e3 < -5 ~ 'green', 
                                      house$Delta / 1e3 > 5 ~ 'red',
                                      TRUE ~ 'orange'))
...
# --------------------------------
  ...
  # ------------------------------
      ...
      output$predicted_price = renderUI({
        ...
        price = paste0('Predicted Price: ', round(predicted_price() / 1000), 'K')
        
        tagList(title, dashboardLabel(price, status = 'primary'))
