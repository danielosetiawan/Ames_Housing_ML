...
df_coefs = read.csv('../shiny_data/final_predictions.csv')
...
locs = read.csv('../../data/Ames_loc.csv')
library(dplyr)
library(readr)
```
...
df %>%
  merge(locs, on=PID) %>%
  summarize(PID, GarageArea, LotArea) %>%
  merge(df_predictions) %>%
  write_csv('./home_flipping.csv')
```
...
```{r}
address = '3110 KINGMAN RD'
property = df_predictions %>%
  filter(Prop_Addr == address)

# df_predictions$Neighborhood
```
...
```{r}
  df_predictions %>%
    filter(distance != 0,
           between(SalePrice, 0.85*df_property()$SalePrice, 1.15*df_property()$SalePrice),
           between(GrLivArea, 0.7*df_property()$GrLivArea, 1.3*df_property()$GrLivArea),
...
df_predictions %>%
  filter(distance != 0,
         between(SalePrice, 0.9*property$SalePrice, 1.1*property$SalePrice),
         between(GrLivArea, 0.8*property$GrLivArea, 1.2*property$GrLivArea),
         # between(LotArea, 0.5*property$LotArea, 1.5*property$LotArea),
         between(OverallQual, property$OverallQual-2, 3+property$OverallQual),
         between(OverallCond, property$OverallCond-2, 3+property$OverallCond)
